...
Persistência dos dados de sensores IoT do campo e das recomendações de manejo geradas
pelo modelo de Machine Learning. Estratégia de **duplo backend** (resolvida em `src/db.py`):

- **Oracle Database (FIAP)** — entrega obrigatória, usado quando há `.env` válido. DDL,
  seed e consultas exploratórias em `src/sql/oracle/`; importação no SQL Developer
  documentada em [`docs/oracle_import.md`](../../docs/oracle_import.md).
- **SQLite** (`farmtech.db`) — _fallback_ local (este `01_schema.sql`), para rodar
  dashboard/ML sem credencial.

Os dois schemas são equivalentes; as colunas `ts` (momento da leitura) e `data_acao`
têm esses nomes para evitar palavras reservadas do Oracle (`timestamp`/`date`).
...
| ------------------- | ------- | ------------------------------------ |
| id                  | INTEGER | PK, autoincremento                   |
| ts                  | TEXT    | Momento da leitura (ISO-8601)        |
...
| id              | INTEGER | PK, autoincremento                      |
| cultura_id      | INTEGER | FK → `culturas.id`                      |
| data_acao       | TEXT    | Data da recomendação (ISO-8601)         |
...
(`chunksize=500`) e imprime a verificação (COUNT por tabela). Popula o SQLite sempre e o
**Oracle FIAP** quando há `.env` válido.
...
Oracle (no SQL Developer, conectado ao FIAP — ver `docs/oracle_import.md`):
```sql
@src/sql/oracle/01_schema_oracle.sql
@src/sql/oracle/02_seed_data.sql
@src/sql/oracle/03_queries_exploratorias.sql
```
